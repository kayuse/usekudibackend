...
@shared_task(bind=True, max_retries=10, default_retry_delay=60)
def fetch_session_transactions(self, account_id: int):
    try:
        db = next(get_db())
        print(f"Starting task to fetch initial transactions for account: {account_id}")
        print(f"Database session: {db}")
...
@celery_app.task(name='auto_classify_session_transactions', bind=True, max_retries=10, default_retry_delay=60)
def auto_classify_session_transactions(self):
