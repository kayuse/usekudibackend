...
from celery.security import register_auth
from dotenv import load_dotenv
import requests
from app.data.mono import MonoAccountBalanceResponse, MonoAccountLinkResponse, MonoAuthResponse, MonoDetailsResponseData
...
class MonoService:
    ...
    def account_auth(self, code: str) -> MonoAuthResponse:
        headers = MonoService.get_header()
        payload = {
            "code": code,  # Generate a unique account ID
        }
    ...
    def fetch_account_details(self, account_id: str) -> MonoDetailsResponseData:
        headers = MonoService.get_header()
        response = requests.get(f'{self.mono_api_base_url}/accounts/{account_id}', headers=headers)
        data: MonoDetailsResponseData = MonoDetailsResponseData(**response.json())
    ...
    def get_transactions(self, account_id: str, start_date: str = None, end_date: str = None, realtime=False) -> List[
        dict]:
        headers = MonoService.get_header()
        if realtime:
